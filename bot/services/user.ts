...
export class UserService {
	...
	public async saveAthleteRuns() {
		const user = await User.findById(this.userId, 'accessToken');
		if (!user) return -1;
		...
		// TODO: somehow make asynchronous??
	...
	public async getLatestRun(): Promise<IRun | null> {
		...
		const runs = await Run.find({ athleteId: user.athleteId }).sort({"startDateLocal" : -1}).limit(1) as IRun[];
